#!/bin/bash
# limit CPU usage when beyond limit （会重复启动限制进程，改成了使用cpulimit_daemon.sh）
...
do
  ...
  # 删除之前的cpulimit进程
  # ps aux | grep cpulimit | grep color | awk '{print $2}' | xargs kill -s 9
  ...
  #done
  # awk '{print $13~/^[0-9]+$/}'
  ps aux | grep -v "cpulimit" | awk '{if($3>55.0 && $1!="d" && $1!="root") print $2}' | while read procid
  do
